...
def draw(action, x, y, flags, userdata):
    ...
    global drawing, zoom
    global img_resized, scale, image_clear_all, image_without_last_element, last_element_type
    ...
    elif action==cv2.EVENT_MOUSEMOVE and zoom: # Update zoom rendering while moving mouse
        zoom_border = int(zoom_window_size[0]/(zoom_multiplier*2))
        if y - zoom_border < 0 and x - zoom_border < 0:
            zoom_frame = img_resized[0 : 2*zoom_border, 0 : 2*zoom_border]
            zoom_frame = cv2.resize(zoom_frame, zoom_window_size)
        elif y - zoom_border < 0 and x + zoom_border > img_resized.shape[1]:
            zoom_frame = img_resized[0 : 2*zoom_border, img_resized.shape[1] - 2*zoom_border : img_resized.shape[1]]
            zoom_frame = cv2.resize(zoom_frame, zoom_window_size)
        elif y + zoom_border > img_resized.shape[0]  and x - zoom_border < 0:
            zoom_frame = img_resized[img_resized.shape[0] - 2*zoom_border : img_resized.shape[0], 0 : 2*zoom_border]
            zoom_frame = cv2.resize(zoom_frame, zoom_window_size)
        elif y + zoom_border > img_resized.shape[0] and x + zoom_border > img_resized.shape[1]:
            zoom_frame = img_resized[img_resized.shape[0] - 2*zoom_border : img_resized.shape[0], img_resized.shape[1] - 2*zoom_border : img_resized.shape[1]]
            zoom_frame = cv2.resize(zoom_frame, zoom_window_size)
        elif y - zoom_border < 0:
            zoom_frame = img_resized[0 : 2*zoom_border, x - zoom_border : x + zoom_border]
            zoom_frame = cv2.resize(zoom_frame, zoom_window_size)
        elif y + zoom_border > img_resized.shape[0]:
            zoom_frame = img_resized[img_resized.shape[0] - 2*zoom_border : img_resized.shape[0], x - zoom_border : x + zoom_border]
            zoom_frame = cv2.resize(zoom_frame, zoom_window_size)
        elif x - zoom_border < 0:
            zoom_frame = img_resized[y - zoom_border : y + zoom_border, 0 : 2*zoom_border]
            zoom_frame = cv2.resize(zoom_frame, zoom_window_size)
        elif x + zoom_border > img_resized.shape[1]:
            zoom_frame = img_resized[y - zoom_border : y + zoom_border, img_resized.shape[1] - 2*zoom_border : img_resized.shape[1]]
            zoom_frame = cv2.resize(zoom_frame, zoom_window_size)
        else:
            zoom_frame = img_resized[y - zoom_border : y + zoom_border, x - zoom_border : x + zoom_border]
            zoom_frame = cv2.resize(zoom_frame, zoom_window_size)
        cv2.imshow('zoom', zoom_frame)
    
    # Action to be taken when right button is pressed while holding SHIFT: open zoom window
...
def initialize(args, welcome=False):
    if welcome:
        ...
        print('Use shift and mouse right button to use a magnifying lens on current window. It automatically closes after releasing mouse button.')
        print('Use space bar to activate dummy function.')
...
def dummy_func(dir_path, image_name, resized_height):
    '''
    Makes a copy of the file and returns the path to the new file
    '''
    file_path = dir_path + '/' + image_name
    new_file_path = dir_path + '/' + image_name[:-4] + '_copy.png'
    shutil.copyfile(file_path, new_file_path)
    _, file_resized, _ = read_and_resize(new_file_path, resized_height)
    return file_resized
...
if __name__ == '__main__':
    ...
    zoom = False # Flag to indicate if zoom mode is on/off
    ...
    while k != 27 and cv2.getWindowProperty(image_name, 0) >= 0:
        if not drawing:
            ...
            window_position[0] -= 9 # to keep window in previous position (x axis)
            window_position[1] -= 32 # to keep window in previous position (y axis)
        ...
        # Space key to call function and display image.
        elif k == 32:
            # image_path = glob.glob(dir_path+'/'+image_name)[0]
            new_image = dummy_func(dir_path, image_name, resized_height)
            cv2.imshow(f'{image_name[:-4]}_copy.png', new_image)
            cv2.moveWindow(f'{image_name[:-4]}_copy.png', 0, 0)
